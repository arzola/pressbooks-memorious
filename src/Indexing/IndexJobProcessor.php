...
namespace PressbooksMemorious\Indexing;

class IndexJobProcessor
{
    public static function register(): void
    {
        add_action('pb_memorious_index_processor', [self::class, 'processQueue']);

        if (! wp_next_scheduled('pb_memorious_index_processor')) {
            wp_schedule_event(time(), 'every_minute', 'pb_memorious_index_processor');
        }
    }
    ...
    public static function enqueueJob(int $blogId, string $jobType, array $payload = []): void
    {
        ...
        $existing = app('db')->table('pressbooks_memorious_index_jobs')
        ...
        if ($existing) {
            app('db')->table('pressbooks_memorious_index_jobs')
                ->where('id', $existing->id)
                ->update(['updated_at' => current_time('mysql', true)]);
            return;
        }

        app('db')->table('pressbooks_memorious_index_jobs')->insert([
            'blog_id' => $blogId,
            'job_type' => $jobType,
            'payload' => $payloadJson,
            'status' => 'pending',
    ...
    {
        return app('db')->table('pressbooks_memorious_index_jobs')
            ->where('status', 'pending')
    ...
    {
        $job = app('db')->table('pressbooks_memorious_index_jobs')->find($jobId);
        if (! $job) {
            return;
        }
        ...
        if ($newAttempts < $maxRetries) {
            ...
            do_action('pb_memorious_job_failed', $jobId, $errorMessage);
        }
    }
    ...
    {
        $client = \PressbooksMemorious\Search\TypesenseClient::fromSettings();
        $payload = $job->payload ? json_decode($job->payload, true) : [];

        $indexers = apply_filters('pb_memorious_indexers', [
        ...
        switch ($job->job_type) {
            ...
            case 'reindex_book':
                do_action('pb_memorious_reindex_started', $job->blog_id);
                try {
                    $client->getClient()->collections['pb_sections']->documents->delete(['filter_by' => "blog_id:{$job->blog_id}"]);
                } catch (\Throwable $e) {
                    // Ignore 404s
                ...
                do_action('pb_memorious_reindex_completed', $job->blog_id);
                break;
        }
    }
}
