...
document.addEventListener('DOMContentLoaded', () => {
			...
			debounceTimer = setTimeout(async () => {
				try {
					const results = await searchClient.search([
						{ indexName: 'pb_contributors', params: { query, hitsPerPage: 3 } },
						{ indexName: 'pb_books', params: { query, hitsPerPage: 3 } },
						{ indexName: 'pb_sections', params: { query, hitsPerPage: 5 } },
					]);
	...
	if (searchPage) {
		...
		search.addWidgets([
			...
			refinementList({ container: '#pb-borges-filter-post-type', attribute: 'post_type' }),
			refinementList({ container: '#pb-borges-filter-book', attribute: 'book_title', searchable: true }),
			refinementList({ container: '#pb-borges-filter-authors', attribute: 'authors', searchable: true }),
			refinementList({ container: '#pb-borges-filter-license', attribute: 'section_license' }),
			hits({
